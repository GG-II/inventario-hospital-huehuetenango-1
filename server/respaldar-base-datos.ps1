...
$origen = ".\server\database.sqlite"  # ← AJUSTADO para tu BD
$carpetaRespaldos = ".\respaldos"
$destino = "$carpetaRespaldos\database_$fecha.sqlite"
...
try {
    ...
    Write-Host ""
    Get-ChildItem $carpetaRespaldos -Filter "database_*.sqlite" | 
        Sort-Object LastWriteTime -Descending | 
        Select-Object -First 5 | 
        Format-Table Name, @{Name="Tamaño (MB)";Expression={[math]::Round($_.Length/1MB, 2)}}, LastWriteTime -AutoSize
    
    # Contar respaldos
    $cantidadRespaldos = (Get-ChildItem $carpetaRespaldos -Filter "database_*.sqlite").Count
    Write-Host ""
